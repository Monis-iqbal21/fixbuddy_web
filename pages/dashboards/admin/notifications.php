...
//
// Works for BOTH client + admin using session role.
// Features:
// - list notifications (paginated)
// - search (title/body/type/job_id)
// - filter (all/unread/read)
// - mark single as read + redirect to link (or fallback)
// - mark all as read
...
function fm_is_internal_link(string $link): bool
{
    $u = trim($link);
    if ($u === '') return false;
    if (stripos($u, 'http://') === 0 || stripos($u, 'https://') === 0) return false; // block external
    if ($u[0] === '/') return true;
...
{
    // if notification link missing, fallback here
...
if (isset($_GET['open'])) {
    ...
    if ($nid > 0) {
        ...
        if ($notif) {
            ...
            $fallback = fm_default_fallback_page($role);

            if ($HAS('link') && $link !== '' && fm_is_internal_link($link)) {
                fm_redirect($link);
            }
            fm_redirect($fallback);
        } else {
            ...
            <?php else: ?>
                <div class="divide-y divide-slate-100">
                    <?php foreach ($notifications as $n): ?>
                        ...
                        $qs = $baseQS;
                        $qs['open'] = $nid;
                        unset($qs['p']); // optional: open without pagination param
