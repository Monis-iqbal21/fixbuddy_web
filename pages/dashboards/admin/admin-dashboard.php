<?php
// admin-dashboard.php
// NOTE: UI is kept EXACTLY the same as your client-dashboard.php.
// Only changes:
// 1) Role guard = admin
// 2) Sidebar links = Dashboard, Jobs (filter types inside jobs page), Add Worker, Notifications, Reviews, Logout
// 3) Page routing points to admin components
...
if (strtolower(trim($dbUser['role'] ?? '')) !== 'admin') {
    fm_redirect_404();
}
...
$adminAvatar = '/fixmate/assets/images/avatar-default.png';
...
$pages = [
    ...
    'jobs'          => './jobs-status.php',      // admin jobs list w/ filters (you’ll create this in admin folder)
    'job-detail'    => './job-detail.php',       // admin job detail (optional)
        ...
        <div class="flex-1 overflow-y-auto py-6 px-4 scrollbar_custom">
            ...
            <div class="mb-6 p-3 bg-gray-50 rounded-xl border border-gray-100">
                <div class="flex items-center gap-3">
                    <img src="<?php echo htmlspecialchars($adminAvatar, ENT_QUOTES, 'UTF-8'); ?>"
                         alt="Profile"
            ...
            <nav class="space-y-1">
                ...
                <a href="admin-dashboard.php?page=jobs"
                   class="link flex items-center gap-3 px-4 py-3 rounded-lg text-gray-600 hover:bg-gray-50 hover:text-black transition-colors <?php echo $page === 'jobs' ? 'active font-semibold' : ''; ?>">
                    <span class="ph-duotone ph-briefcase text-xl"></span>
                    <span>Jobs</span>
                </a>

                <a href="admin-dashboard.php?page=add-worker"
                   class="link flex items-center gap-3 px-4 py-3 rounded-lg text-gray-600 hover:bg-gray-50 hover:text-black transition-colors <?php echo $page === 'add-worker' ? 'active font-semibold' : ''; ?>">
                    <span class="ph-duotone ph-user-plus text-xl"></span>
                    <span>Add a Worker</span>
                </a>
                <a href="admin-dashboard.php?page=workers-list"
                   class="link flex items-center gap-3 px-4 py-3 rounded-lg text-gray-600 hover:bg-gray-50 hover:text-black transition-colors <?php echo $page === 'workers-list' ? 'active font-semibold' : ''; ?>">
                    <span class="ph-duotone ph-users text-xl"></span>
                    <span>Workers List</span>

                <a href="admin-dashboard.php?page=notifications"
    ...
    <main class="flex-1 flex flex-col h-full relative overflow-hidden">
        ...
        <header class="lg:hidden bg-white h-16 flex items-center justify-between px-4 border-b border-gray-200 flex-shrink-0">
            <div class="flex items-center gap-3">
                ...
                <span class="font-bold text-lg text-gray-800">fix Buddy</span>
            </div>
            <div class="w-8 h-8 rounded-full overflow-hidden border border-gray-200">
                <img src="<?php echo htmlspecialchars($adminAvatar, ENT_QUOTES, 'UTF-8'); ?>" class="w-full h-full object-cover">
            </div>
        </header>
            ...
            <div class="max-w-9xl mx-auto">
                <div class="mb-6">
                    <h1 class="text-2xl font-bold text-gray-800">
                        Hello, <span class="text-indigo-600"><?php echo htmlspecialchars($adminName, ENT_QUOTES, 'UTF-8'); ?></span>
                    </h1>
                ...
                <div class="mt-8 py-6 text-center border-t border-gray-200 text-xs text-gray-400">
                    &copy;<?php echo date('Y'); ?> Fix Buddy. All Rights Reserved.
                </div>
            </div>
