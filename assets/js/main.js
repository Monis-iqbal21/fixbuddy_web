...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.nav_item > a').forEach(link => {
        link.addEventListener('click', e => {
            ...
            if (subMenu && (subMenu.classList.contains('sub_nav_mobile') || subMenu.classList.contains('sub_nav_mobile2'))) {
                e.preventDefault();
    ...
    // Back buttons for all submenus
    document.querySelectorAll('.back_btn').forEach(btn => {
        btn.addEventListener('click', e => {
            const parentSubMenu = btn.closest('.sub_nav_mobile, .sub_nav_mobile2');
            if (parentSubMenu) {
